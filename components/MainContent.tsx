...
import { Database, Building2, Trophy, ArrowUp, Search, X } from 'lucide-react';
        ...
        <div className="w-full max-w-2xl relative" ref={wrapperRef}>
          <div className="relative flex items-center">
            <input 
              ...
              placeholder="输入指标名称，输入 '指标空间' 可直接跳转" 
              className="w-full h-12 pl-6 pr-12 rounded-full border border-blue-200 shadow-sm text-gray-700 placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all"
            />
            
            {inputValue && (
              <button 
                onClick={() => { setInputValue(''); setShowSuggestions(false); }}
                className="absolute right-4 text-gray-300 hover:text-gray-500 transition-colors p-1"
              >
                <X size={18} />
              </button>
            )}
